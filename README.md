...
```bash
uv run python cli.py --launch-browser "your search query"
```
...
| `--initial-wait` | 10.0 | Seconds to wait after page load |
| `--hover-delay` | 1.0 | Seconds to wait after hover |
...
```bash
uv run python cli.py --launch-browser --count 10 --output-json results.json "cats"
```

Keep browser open after scraping:

```bash
uv run python cli.py --launch-browser --on-finish keep "dogs"
```
